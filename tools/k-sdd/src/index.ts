...
const helpText = `Usage: k-sdd [options]

Options:
  --agent <${agentKeys.join('|')}>  Select agent
${agentAliasLine}  --lang <ko|en|zh-TW|zh|es|pt|de|fr|ru|it|ko|ar|el>  Language
...
const runPlanExecution = async (
  ...
  try {
    ...
    io.log('');
    io.log(formatBox(`k-sdd v${version} / ${agentDef.label}`));
...
export const runCli = async (
  ...
  if (parsedArgs.agent === 'codex') {
    ...
    io.log('');
    io.log(`  ${colors.bold('npx k-sdd@latest --codex-skills')}`);
    io.log('');
    return 1;
  }
